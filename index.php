...
function format_ribuan ($nilai){
                ...
                <input type="hidden" class="form-control" value="<?php echo $tot_bayar; ?>" id="hargatotal" readonly>
                    <label class="col-sm-4 col-form-label col-form-label-sm"><b>Bayar</b></label>
                        <div class="col-sm-8 mb-2">
                            ...
                            name="bayar" id="bayarnya" oninput="totalnya()" required">
                    </div>
    ...
    $notrans = mysqli_real_escape_string($conn, $_POST['no_transaksi']);
    $bayar = (int)$_POST['bayar'];
    $kembalian = (int)$_POST['kembalian'];

     if ($bayar <= 0) {
        echo '<script>alert("Harap memasukkan uang pembayaran."); window.location="index.php"</script>';
        exit;
    }

    if ($bayar < $tot_bayar) {
    echo '<script>alert("Uang bayar kurang dari total belanja."); window.location="index.php"</script>';
    exit;
}
    $sql = "UPDATE keranjang SET no_transaksi='$notrans', bayar='$bayar', kembalian='$kembalian'";
    $query = mysqli_query($conn, $sql);

    echo '<script>alert("Pembayaran berhasil. Silakan klik Selesai untuk menyimpan ke laporan."); window.location="index.php"</script>';
...
if (isset($_POST['selesai'])) {
    // Ambil total subtotal dari keranjang
    $queryTotal = "SELECT SUM(subtotal) as total_belanja FROM keranjang WHERE no_transaksi IS NOT NULL";
    $resultTotal = mysqli_query($conn, $queryTotal);
    $dataTotal = mysqli_fetch_assoc($resultTotal);
    $totalBelanja = $dataTotal['total_belanja'];

    if ($totalBelanja <= 0) {
        // Jika total belanja 0, tampilkan notifikasi
        echo '<script>alert("Data laporan tidak valid"); window.location="index.php"</script>';
    } else {
        // Proses pemindahan data ke tabel laporanku
        $query = "SELECT * FROM keranjang WHERE no_transaksi IS NOT NULL";
        $result = mysqli_query($conn, $query);

        while ($data = mysqli_fetch_assoc($result)) {
            $insert = "INSERT INTO laporanku 
            (no_transaksi, bayar, kembalian, id_cart, kode_barang, nama_barang, harga_barang, quantity, subtotal, tgl_input)
            VALUES (
                '{$data['no_transaksi']}',
                '{$data['bayar']}',
                '{$data['kembalian']}',
        ...
        echo '<script>alert("Transaksi selesai. Data telah masuk laporan."); window.location="index.php"</script>';
    }
}
?>
  </div>
...
function total() {
  ...
  function totalnya() {
    var bayar = parseInt(document.getElementById('bayarnya').value) || 0;
    var total = parseInt(document.getElementById('hargatotal').value) || 0;
    var kembali = bayar - total;
    document.getElementById('total1').value = kembali >= 0 ? kembali : 0;
}
